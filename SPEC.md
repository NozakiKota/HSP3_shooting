...
| 7 | 無駄パターン検出（3種） | ✅ 実装済み |
| 8 | 自動化提案（3種） | ✅ 実装済み |
| 9 | Google カレンダー連携 | ✅ 実装済み |
| 10 | ML ベースのパターン検出（統計的手法） | ✅ 実装済み |
| 11 | 起動時の過去ログ復元 | ✅ 実装済み |
| 12 | ログ保存先の絶対パス化 | ✅ 実装済み |
| 13 | teams/zoom の会議分類精度向上 | ✅ 実装済み |
...
└──────────┬──────────────────────────┬──────────────────────────┘
           ...
           │ load_records()           │ → list[ActivityRecord]
           ▼                          ▼
┌──────────────────────┐    ┌─────────────────────────────────────┐
│   logger.py          │    │   pattern_detector.py               │
│   ActivityLogger     │    │   analyze()                         │
│   DEFAULT_OUTPUT_DIR │    │   detect_work_sessions()            │
│   load_from_csv()    │    │   score_productivity()              │
│                      │    │   detect_peak_hours()               │
...
│  └───────┬────────┘  │    ┌─────────────────────────────────────┐
│          │           │    │   google_calendar.py                │
│    win32gui/psutil   │    │   CalendarClient                    │
│    でウィンドウ取得   │    │   link_activities_to_events()       │
...
└──────────────────────┘
           │
           ▼
    ~/.windows_activity_logger/logs/activity_YYYYMMDD_HHMMSS.csv
```
...
| # | 制限 | 影響 | 対応状況 |
|---|------|------|---------|
| 1 | ポーリング間隔が5秒のため、5秒未満のウィンドウ切り替えは記録されない | 瞬間的なウィンドウ切り替えが欠落する可能性 | 🔜 `SetWinEventHook` によるイベント駆動方式に変更予定 |
| 2 | カテゴリ分類はキーワード部分一致のみ | 未登録アプリは全て `other` になる | 🔜 ユーザー定義カテゴリ設定の追加予定 |
| 3 | `teams` / `zoom` の会議中判定が不正確だった | 会議時間を `meeting` として区別できなかった | ✅ ウィンドウタイトルの会議キーワード一致で `meeting` に分類するよう修正済み |
| 4 | UI の `_refresh_summary` / `_refresh_insights` はウィジェットを毎回全削除・再生成する | レコード数が増えると UI 更新がわずかに重くなる | 🔜 差分更新またはキャッシュの導入予定 |
| 5 | ログは `logs/` ディレクトリへの相対パスで保存されていた | `main.py` の実行ディレクトリ依存 | ✅ `~/.windows_activity_logger/logs/` への絶対パスに変更済み |
| 6 | 非 Windows 環境ではモックデータ `("MockApp", "Mock Window Title")` が固定で返る | 実際の操作ログは収集できない | 🔜 X11/Wayland 対応や macOS 対応の追加予定 |
| 7 | CSV は追記専用で、過去セッションのデータは UI に読み込まれなかった | アプリを再起動すると過去ログが表示されなかった | ✅ 起動時に直前セッションの CSV を自動読み込みするよう修正済み |
